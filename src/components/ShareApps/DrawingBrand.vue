...
export default {
  ...
  methods: {
    handleSentDrawingMsg (res) {
      console.log('handle send drawing msg result: ', res)
    },
    ...
    handleMucUserChange () {
      ...
      Utils.onMucUserChange(function (res) {
        ...
        let index = _this.mucUsers.indexOf(jid)
        if (res.action === 'entered') {
          ...
          // remove jid from list
          if (index !== -1) {
            _this.mucUsers.splice(index, 1)
          }
        }
      })
    },
    ...
    handleDrawingMsg () {
      let _this = this
      Utils.onGetDelayDrawing(function (res) {
        if (res.action === 'message') {
          let urlData = JSON.parse(res.data)
          // console.log('=====================handleGetDelayDrawingMsg=================:', urlData, _this.lineTracks.length)
          if (urlData.type === 'getInit') {
            let receiver = res.from
            for (let i = 0; i < _this.lineTracks.length; i++) {
              let item = _this.lineTracks[i]
              _this.sendActionMsg(item.action, item.data, item.type, receiver, true)
            }
          }
        }
      })
